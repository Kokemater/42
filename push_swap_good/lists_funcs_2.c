...
/*   Updated: 2025/02/11 13:46:59 by jbutragu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	swap_consecutive_nodes(t_list **head, t_list *a, t_list *b, t_list *prev)
{
	if (a->next == b) // Si `a` está antes que `b`
	{
		if (prev)
			prev->next = b;
		else
	...
	else if (b->next == a) // Si `b` está antes que `a`
	{
		if (prev)
			prev->next = a;
		else
...
void	lst_swap_position(t_list **head, t_list *node_a, t_list *node_b)
{
	...
	{
		prev = lst_prev_node(*head, (node_a->next == node_b) ? node_a : node_b);
		swap_consecutive_nodes(head, node_a, node_b, prev);
		return ;
	}

	t_list *node_a_prev = lst_prev_node(*head, node_a);
	t_list *node_b_prev = lst_prev_node(*head, node_b);

...
{
	if (!p->dead && p->last_eat < (timestamp_ms() - p->s->time_death))
	{
		print_status(p, "died");
		p->dead = 1;
		p->s->should_end = 1;
...
static int	should_end(t_philo *d, int unlock)
{
	...
	ret = 0;
	pthread_mutex_lock(&d->s->check_mut);
	if (d->s->should_end)
	{
		ret = 1;
		if (unlock & UNLOCK_L)
	...
	pthread_mutex_unlock(&d->s->check_mut);
	return (ret);
}
